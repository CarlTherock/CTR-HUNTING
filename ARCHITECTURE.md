...
│   │
│   ├── services/                External API adapters — map/ (Phase 1: MapProvider, MapLibreProvider); see services/README.md
...
| MapTiler as the map tile provider (Phase 1, slice 1.1) | Free tier is sufficient for development; the "Outdoor" style bundles satellite, topographic and contour rendering in one style URL instead of three separate ones. Its terrain-RGB tiles are reused for elevation in Phase 4 (3D terrain) and Phase 8 (Analytics Engine), avoiding a second elevation provider until one is actually needed. Hidden entirely behind `MapProvider`/`MapLibreProvider` (`src/services/map/`), so swapping providers later touches one file, not feature code.                                                                                                                                    |
| Esri as a second map base-layer vendor (post-Phase-1) | User request for more base layer variety (terrain relief, neutral canvases for future analytics, road navigation) beyond MapTiler's two styles. Both vendors emit MapLibre-compatible style JSON, so `MapTilerProvider` was renamed/generalized to `MapLibreProvider` rather than adding a second `MapProvider` implementation — one MapLibre engine instance, two vendors of style URL. Esri's Basemap Styles v2 free tier (2M tiles/month, then $0.15/1,000, no charge possible without a payment method on file) matches MapTiler's risk profile; the key is scoped to a "Public app" with only the "Basemaps" privilege, same reasoning as any client-embedded key. |
