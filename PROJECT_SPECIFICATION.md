...
| 6   | Wind ✅                | Dedicated wind engine: direction, speed, gusts, temporal change, map animation, interactive timeline. Provider: Open-Meteo (`WindProvider` adapter, keyless, batched grid fetch, verified live). Animated particle flow-field (canvas over the map, real grid samples, never interpolated), 24h timeline scrubber; plus a per-waypoint "Optimal Wind" octant picker + live match badge, beyond the base spec (inspired by onX Hunt's own "Optimal Wind" feature). |
